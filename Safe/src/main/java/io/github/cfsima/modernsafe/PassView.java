...
public class PassView extends AppCompatActivity implements SimpleGestureFilter.SimpleGestureListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (debug) {
            Log.d(
                    TAG, "RowId=" + RowId + " CategoryId=" + CategoryId + " rowids=" + rowids +
        ...
        if (debug) {
            Log.d(TAG, "rowids.length=" + rowids.length);
        }
    ...
    private void initFlipper() {
        ...
        if (rowids.length > (listPosition + 1)) {  // is there a next?
            ...
            if (debug) {
                Log.d(TAG, "flipper ChildCount=" + flipper.getChildCount());
            }
        ...
        if (debug) {
            Log.d(TAG, "flipper.getChildCount=" + flipper.getChildCount());
        }
    }
    ...
    private void showPreviousView(boolean isRightLeft) {
        ...
        if (debug) {
            Log.d(TAG, "previousButton displayedChild=" + flipper.getDisplayedChild() + " listPosition=" + listPosition);
        }
    ...
    private boolean populateFields(long rowIdx, View view) {
        if (debug) {
            Log.d(TAG, "populateFields(" + rowIdx + "," + view + ")");
        }
        if (rowIdx > 0) {
            if (debug) {
                Log.d(TAG, "rowIdx=" + rowIdx);
            }
            ...
            if (row.plainUniqueName != null) {
                uniqueNameText.setText(
                        getString(R.string.uniquename) +
                                ": " + row.plainUniqueName
                );
            }
            String packages = "";
            ...
            if (packages != "") {
                packageAccessText.setText(
                        getString(R.string.package_access) +
                                ": " + packages
                );
            }
        }
